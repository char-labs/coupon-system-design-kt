...
현재 저장소는 `requestId`를 record key로 사용한다.
...
현재 쿠폰 시스템에서는 `POST /coupon-issue-requests` 가 이 목적을 가진다.
...
sequenceDiagram
    ...
    Client->>API: POST /coupon-issue-requests
...
- 그래서 현재 저장소는 `POST /coupon-issue-requests` 에서 `202 Accepted`를 반환한다.
...
`POST /coupon-issue-requests`는 짧게 끝나고, 실제 발급은 worker가 처리한다.
