...
- publish용 글과 외부 reference pack을 분리한다
...
- planner
  ...
  - 외부 `근거 팩` 과 publish용 `초안` 을 분리해 계획한다
  - publish용 MDX frontmatter와 대표 이미지 적용 여부를 먼저 결정한다
...
- drafter
  ...
  - publish용 `초안` 과 외부 `근거 팩` 을 따로 작성
  - publish용 `초안` 이 MDX면 frontmatter와 대표 이미지까지 포함해 바로 게시 가능한 형태로 작성한다
  - 본문 내 `이미지 제안:` 블록 포함
  ...
  - 플랫폼이 허용하면 `<a href="#ref-e1">[E1]</a>` 와 대응 `id` 를 가진 `References` 를 사용할 수 있다
- reviewer
  ...
  - frontmatter, 대표 이미지, inline citation anchor가 있으면 깨진 부분이 없는지 확인한다
...
- `근거 팩` 은 유지하되 `External References` 와 필요한 경우 `Writing Benchmarks` 중심으로 구성한다
...
- 외부 근거를 문장 중간에 자주 연결해야 하면 anchor 기반 reference 표기를 허용한다

`근거 팩` 은 기본적으로 아래 카테고리로 구성한다.
...
즉, publish용 본문은 `이 사실이 어디 코드에서 왔는가` 를 독자가 이해할 만큼만 드러내고, 파일 경로 나열은 최종 산출물에서 생략한다.

## Publish 하네스 규칙

publish용 MDX에서는 아래 하네스를 기본값으로 본다.
...
- frontmatter
  ...
  - `date`
  - `tags`
- 대표 이미지가 있으면 frontmatter 직후 배치
- 본문은 `서론 -> 문제 제기 -> 개념 -> 구조 고정 -> 코드 설명 -> 실패 처리 -> 최종 결정 -> 결론` 흐름을 우선 검토
- external reference를 inline으로 연결해야 하면 `<a href="#ref-e1">[E1]</a>` 와 `<span id="ref-e1">[E1]</span>` 조합을 사용할 수 있다
